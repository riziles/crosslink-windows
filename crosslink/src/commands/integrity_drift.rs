...
//! paths that close the gap by writing SQLite-only rows back to the JSON
//! event log via `SharedWriter` (#602).
//!
...
//! contents — most importantly: SQLite has a row (a label, a blocker, a
//! relation) that no JSON file represents. The repair path used to
...
//! - [`re_emit`] — for each re-emittable category, writes the SQLite-only
//!   rows back to the JSON / git event log via `SharedWriter`.
//!
...
/// Categorized record of every SQLite row that is not represented in the
/// hydrated-from-JSON view of state.
///
...
pub struct HydrationDriftReport {
    ...
    /// (issue_display_id, label) pairs present in SQLite but not in JSON,
    /// restricted to issues that DO appear in JSON. Re-emittable via
    /// `SharedWriter::add_label`.
    pub sqlite_only_labels: Vec<(i64, String)>,

    /// (blocker_display_id, blocked_display_id) — blocker dependencies
    /// in SQLite but not in JSON, restricted to JSON-known issues on
    /// both sides. Re-emittable via `SharedWriter::add_blocker`.
    pub sqlite_only_dependencies: Vec<(i64, i64)>,

    /// (issue_a_display_id, issue_b_display_id) — relations in SQLite
    /// but not in JSON, canonicalized as `(min, max)` because SQLite
    /// stores both directions while JSON stores one. Re-emittable via
    /// `SharedWriter::add_relation`.
    pub sqlite_only_relations: Vec<(i64, i64)>,

    /// (milestone_display_id, issue_display_id) — milestone assignments
    /// in SQLite that don't appear as `milestone_uuid` on the JSON
    /// issue. Re-emittable via `SharedWriter::set_milestone_on_issues`.
    ...
    /// Time-entry ids in SQLite (on JSON-known issues) that would be
    /// destroyed by `clear_shared_data`. Time entries have no JSON
...
impl HydrationDriftReport {
    /// True when SQLite and the JSON-derived view agree on every row of
    /// every shared table.
    #[must_use]
    ...
    ///
    /// Currently: comments and time entries on JSON-known issues. Issue
    /// rows with `created_by = NULL` are preserved by the existing
    /// `hydrate_to_sqlite` self-heal path; they are not counted as
    /// "unrecoverable" here.
    #[must_use]
    pub fn has_unrecoverable_loss(&self) -> bool {
    ...
    #[must_use]
    pub fn is_fully_re_emittable(&self) -> bool {
        !self.is_empty()
            && self.sqlite_only_comments.is_empty()
            && self.sqlite_only_time_entries.is_empty()
            && self.sqlite_only_issues.is_empty()
...
/// Diff every shared SQLite table against the JSON-derived view of the
/// same state. Returns a categorized record of every row that exists in
/// SQLite but not in JSON.
///
...
mod tests {
    ...
    /// Look up an issue's JSON file by display id by scanning the hub
    /// cache. Used in tests so we can inspect post-re-emit JSON state
    /// without depending on private `SharedWriter` helpers.
    fn read_issue_by_display_id(
        cache_dir: &std::path::Path,
        display_id: i64,
    ) -> crate::issue_file::IssueFile {
        let issues = crate::issue_file::read_all_issue_files(&cache_dir.join("issues")).unwrap();
        issues
            .into_iter()
            .find(|i| i.display_id == Some(display_id))
            .unwrap_or_else(|| panic!("no JSON issue file with display_id = {display_id}"))
    }

    /// Spin up a minimal git environment (work repo + bare remote +
    /// initialized hub cache) so `SharedWriter::new()` succeeds, mirroring
    /// `shared_writer::tests::integration::setup_shared_writer_env`.
    /// Inlined here because `crate::commands` lives in the bin target
    /// while that helper lives in the lib target.
    fn setup_writer_env() -> (tempfile::TempDir, tempfile::TempDir, std::path::PathBuf) {
        use crate::identity::{AgentConfig, AgentRole};
        use std::process::Command;
        use tempfile::TempDir;

        let remote_dir: TempDir = tempfile::tempdir().unwrap();
        let work_dir: TempDir = tempfile::tempdir().unwrap();
        ...
        let agent_config = AgentConfig {
            ...
            description: Some("integrity_drift test agent".to_string()),
    ...
    #[test]
    fn test_re_emit_writes_sqlite_only_dependency_to_json() {
        // End-to-end regression for the bug-report scenario: two issues
        // are in JSON (created via SharedWriter), a dependency row is
        // injected directly into SQLite (bypassing the writer), then
        // detect + re_emit must produce a clean JSON-side state with
        // the dep written back through the writer. After re_emit, drift
        // detection must report no remaining divergence.
        let (work_dir, _remote, crosslink_dir) = setup_writer_env();
        let writer = crate::shared_writer::SharedWriter::new(&crosslink_dir)
            .unwrap()
            .unwrap();
        let db = Database::open(&work_dir.path().join("issues.db")).unwrap();

        let blocked = writer
            .create_issue(&db, "needs another", None, "medium", None, None)
            .unwrap();
        let blocker = writer
            .create_issue(&db, "the blocker", None, "high", None, None)
            .unwrap();

        // Inject the SQLite-only dependency.
        db.add_dependency(blocked, blocker).unwrap();

        let cache_dir = crosslink_dir.join(crate::sync::HUB_CACHE_DIR);
        let before = read_issue_by_display_id(&cache_dir, blocked);
        assert!(
            before.blockers.is_empty(),
            "precondition: JSON must not yet contain the SQLite-only blocker"
        );

        let drift = detect(&cache_dir, &db).unwrap();
        assert_eq!(
            drift.sqlite_only_dependencies,
            vec![(blocker, blocked)],
            "detect must surface the SQLite-only dependency, got: {drift:?}"
        );
        assert!(drift.is_fully_re_emittable());
        assert!(!drift.has_unrecoverable_loss());

        let stats = re_emit(&drift, &writer, &db).unwrap();
        assert_eq!(stats.dependencies, 1, "re-emit must write the dep to JSON");

        let after = read_issue_by_display_id(&cache_dir, blocked);
        assert_eq!(
            after.blockers.len(),
            1,
            "JSON must now contain the re-emitted blocker"
        );

        let drift_after = detect(&cache_dir, &db).unwrap();
        assert!(
            drift_after.is_empty(),
            "after re-emit, drift must be empty; got: {drift_after:?}"
        );
