...
![](../assets/img/design-flow.svg)

```
/design "add batch retry logic"
    |
    +-- 1. Explore codebase     (find related modules, types, patterns)
    +-- 2. Interview             (3-5 questions grounded in findings)
    +-- 3. Draft                 (.design/add-batch-retry-logic.md)
    +-- 4. Validate              (requirements, criteria, real file refs)
    +-- 5. Store as knowledge    (crosslink knowledge, tagged design-doc)
```
